import{t as e}from"./rolldown-runtime-Dh6celcD.js";var t,n,r,i,a;function o(){return(o=e((()=>{t={title:`Overview`,parameters:{layout:`padded`,docs:{description:{component:`Overview — what this system is, the laws that hold it together, and the
standing consistency audit. Read this page before adding anything.`}}}},n={name:`The system`,render:()=>`
  <div style="max-width:72ch">
    ...
    <p class="section-lead" style="color:var(--pl-fg-secondary)">Foundations are generated from the stylesheet at
  ...
  </div>`},r={name:`System laws`,render:()=>{let e=(e,t,n)=>`
      <li style="background:var(--pl-surface);border:1px solid var(--pl-border);
        ...
        <strong style="color:var(--pl-fg)">${e}</strong> ${t}
        <span style="display:block;font:500 11px/1.6 ui-monospace,Menlo,monospace;color:var(--pl-fg-muted);margin-top:4px;direction:ltr;unicode-bidi:isolate;text-align:left">${n}</span>
      </li>`;return`<ul style="display:grid;gap:8px;padding:0;margin:0;max-width:78ch">
      ${e(`A button is never narrower than its height.`,`min-width equals height on every control, so an icon-only button is a perfect square.`,`.btn { min-width: var(--ctl-md) } and .btn--icon`)}
      ${e(`Four control heights, no fifth.`,`Every interactive control sits on the ladder: 32, 40, 48 or 56.`,`--ctl-xs / --ctl-sm / --ctl-md / --ctl-lg`)}
      ${e(`No thin text.`,`Body is medium 500 and headings are bold 700. Weights under 500 do not exist here.`,`body { font-weight: 500 } and h1 to h4 { font-weight: 700 }`)}
      ${e(`Content ink is near black.`,`Gray is reserved for true meta: kickers, captions, roles. Never for text somebody has to read.`,`--pl-fg-body for content, --pl-fg-secondary for meta`)}
      ${e(`Crisp edges.`,`Borders are visible and distinct. Nothing washed out.`,`--pl-border and --pl-border-strong`)}
      ${e(`Never show a tooltip while its own menu is open.`,`Suppressed once, globally, not patched per component.`,`[data-tooltip][aria-expanded=true] pseudo-elements`)}
      ${e(`Every clickable thing says so.`,`Pointer cursor set once as a global base rule; disabled shows not-allowed.`,`one global cursor rule`)}
      ${e(`8 px spacing grid,`,`with a single 4 px half-step for tight optical gaps. No ad hoc rem values.`,`--space-1 to --space-7`)}
      ${e(`A corner is a radius rung.`,`4, 8, 16, 24 or 32. Nothing in between.`,`--radius-sm to --radius-2xl`)}
      ${e(`The wordmark never wraps.`,`Product Lab is one unbreakable unit everywhere, and never mirrors in RTL.`,`white-space:nowrap and width:max-content`)}
      ${e(`Hebrew and English ship together.`,`Any component or copy change lands in both languages, and the layout holds in both directions.`,`I18N.he plus I18N.en, logical properties`)}
      ${e(`No long dashes, anywhere.`,`Separation is a small accent dot. This one is a ship gate.`,`eyebrow dot, house law 2026-08-06`)}
    </ul>`}},i={name:`Button audit`,render:()=>{let e={SYSTEM:`#15803d`,KEEP:`#15803d`,DEPRECATE:`#b4443a`,EXCEPTION:`#8a5f14`,WATCH:`#8a5f14`};return`
  <div style="max-width:82ch">
    ...
    <table style="width:100%;border-collapse:collapse;background:var(--pl-surface);
      ...
      <tr style="background:var(--pl-bg)">
        ${[`Variant`,`Used for`,`Verdict`].map(e=>`<th style="text-align:start;padding:8px 12px;border-bottom:1px solid var(--pl-border)">${e}</th>`).join(``)}
      </tr>
      ${[[`.btn`,`Base: 48 high, radius 8, weight 600, min-width equals height`,`SYSTEM`],[`.btn--primary`,`Ink fill. The one strong action on a surface`,`KEEP`],[`.btn--ghost`,`Outline. Secondary actions`,`KEEP`],[`.btn--accent`,`Violet fill. The hero registration CTA`,`KEEP`],[`.btn--wa-solid`,`WhatsApp green. The booking CTA`,`KEEP`],[`.btn--wa`,`Old WhatsApp outline. No markup references it any more; only the CTA-band override keeps it alive`,`DEPRECATE`],[`.login__google`,`Google sign-in only. A third-party logo needs neutral ground`,`EXCEPTION`],[`.btn--sm / .btn--lg`,`Ladder size modifiers, 40 and 56`,`SYSTEM`],[`.btn--icon`,`Any icon-only control. Square at its own rung`,`SYSTEM`],[`.hero__cta .btn`,`Fluid clamp height in container units, so the hero CTA sits off the ladder by design`,`WATCH`]].map(t=>`<tr>
        <td style="padding:7px 12px;border-bottom:1px solid rgb(10 10 10 / .08);font:600 12px ui-monospace,Menlo,monospace;white-space:nowrap;direction:ltr;unicode-bidi:isolate;text-align:left">${t[0]}</td>
        <td style="padding:7px 12px;border-bottom:1px solid rgb(10 10 10 / .08)">${t[1]}</td>
        <td style="padding:7px 12px;border-bottom:1px solid rgb(10 10 10 / .08);font-weight:700;color:${e[t[2]]}">${t[2]}</td>
      </tr>`).join(``)}
    </table></div>
  ...
  render: () => {
    ...
    const rows = [[".btn", "Base: 48 high, radius 8, weight 600, min-width equals height", "SYSTEM"], [".btn--primary", "Ink fill. The one strong action on a surface", "KEEP"], [".btn--ghost", "Outline. Secondary actions", "KEEP"], [".btn--accent", "Violet fill. The hero registration CTA", "KEEP"], [".btn--wa-solid", "WhatsApp green. The booking CTA", "KEEP"], [".btn--wa", "Old WhatsApp outline. No markup references it any more; only the CTA-band override keeps it alive", "DEPRECATE"], [".login__google", "Google sign-in only. A third-party logo needs neutral ground", "EXCEPTION"], [".btn--sm / .btn--lg", "Ladder size modifiers, 40 and 56", "SYSTEM"], [".btn--icon", "Any icon-only control. Square at its own rung", "SYSTEM"], [".hero__cta .btn", "Fluid clamp height in container units, so the hero CTA sits off the ladder by design", "WATCH"]];
    return \`
  <div style="max-width:82ch">
    <span class="eyebrow">Consistency audit</span>
    <h2 style="font-size:1.4rem;margin-top:.4rem">One base, four intents, one dead variant</h2>
...
}`,...i.parameters?.docs?.source}}},a=[`TheSystem`,`SystemLaws`,`ButtonAudit`]})))()}o();export{i as ButtonAudit,r as SystemLaws,n as TheSystem,a as __namedExportsOrder,t as default};