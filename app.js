...
const I18N = {
  he: {
    ...
    hero_title_a: "להפוך לצוות של אחד. עולם חדש של עבודה עם ",
    hero_title_mark: "סוכני AI",
    hero_title_b: ".",
    hero_sub: "ב-3 שעות תקימו צוות סוכני AI משלכם עם Claude שילווה אותכם בדרך החדשה שלכם. ותתחילו לבנות. בזמן אמת.",
    hero_points: ["זיכרון משותף", "בלי קוד", "כוח־על של בנייה"],
    session: {
      badge: "המפגש הבא",
      when_label: "מתי?",
    ...
    proof_self_b: "את הדף הזה, ואת כל הסדנה, בניתי עם אותו סוג של צוות AI שתקימו בעצמכם.",
  ...
  en: {
    ...
    hero_title_a: "Become a team of one. A new world of working with ",
    hero_title_mark: "AI agents",
    hero_title_b: ".",
    hero_sub: "In 3 hours, create your own AI agent team with Claude that will walk this new road with you. And start building. Live.",
    hero_points: ["Shared memory", "No code", "Builder superpower"],
    session: {
      badge: "Next session",
      when_label: "When?",
    ...
    proof_self_b: "This page, and the whole workshop, I built with the same kind of AI team you'll set up yourself.",
...
function wireWhyCursors() {
  ...
  const px = (b) => rnd(MG, Math.max(MG + 1, b.w - 30));
  const py = (b) => rnd(MG, Math.max(MG + 1, b.h - 34));

  const S = els.map((el) => {
    const b = box(), x = px(b), y = py(b);
    return { el, ax: x, ay: y, bx: x, by: y, t: 1, dur: 1, curve: 0, mode: "pause", until: 0, seed: Math.random() * 1000 };
  });
  S.forEach((s) => place(s.el, s.ax, s.ay));

  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    const b = box();                              // static, evenly scattered
    S.forEach((s, i) => place(s.el, px(b), (b.h / S.length) * (i + 0.5)));
    return;
  }
  ...
  const segment = (s, now) => {
    ...
    s.ax = s.bx; s.ay = s.by;
    s.bx = px(b); s.by = py(b);
    s.dur = rnd(1600, 3400); s.curve = rnd(-0.3, 0.3); s.t = 0; s.mode = "move"; s.start = now;
  };
